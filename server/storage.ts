...
export class DatabaseStorage implements IStorage {
  ...
  // Helper: upsert view tracking row and return whether it was newly inserted
  async upsertViewTracking(userId: string, type: string, identifier: string): Promise<{ tracking: ViewTracking; inserted: boolean }> {
    const sqlText = `
      INSERT INTO view_tracking (user_id, type, identifier, last_viewed_at)
      VALUES ($1, $2, $3, NOW())
      ON CONFLICT (user_id, type, identifier)
      DO UPDATE SET last_viewed_at = NOW()
      RETURNING id, user_id, type, identifier, last_viewed_at, (xmax = 0) as inserted
    `;

    const result: any = await (pool as any).query(sqlText, [userId, type, identifier]);
  ...
  // Update view tracking (keeps API backwards compatible) - returns the tracking record
  async updateViewTracking(userId: string, type: string, identifier: string): Promise<ViewTracking> {
    const { tracking } = await this.upsertViewTracking(userId, type, identifier);
    return tracking;
  }
  ...
  async trackTrendView(userId: string, trendId: string): Promise<void> {
    // Use upsert to avoid race conditions and duplicate rows. Only increment trend views when a new row was inserted.
    const { inserted } = await this.upsertViewTracking(userId, "trend", trendId);
    if (inserted) {
      await db
        .update(schema.trends)
  ...
  async getTrendAnalytics(trendId: string): Promise<any> {
    ...
    const totalComments = (await this.getCommentsByTrend(trendId)).length;
    const averageVotesPerPost = totalPosts > 0 ? totalVotes / totalPosts : 0;

    const rankedPosts = await this.getRankedPostsForTrend(trendId);
    const topPosts = rankedPosts.slice(0, 3).map((post) => ({
      id: post.id,
    ...
    // Engagement rate: votes per participant expressed as a percentage
    const engagementRate = uniqueParticipants > 0 ? (totalVotes / uniqueParticipants) * 100 : 0;
